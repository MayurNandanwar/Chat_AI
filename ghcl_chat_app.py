...
app.secret_key = b'009a51c96e4afc04a4085e4999b2b2c4a7c4f12f1659f6f1788586e499ed20d7'
app.config["PERMANENT_SESSION_LIFETIME"] = 86400
app.config["SESSION_TYPE"] = "redis"
app.config["SESSION_PERMANENT"] = False  # Expire session when browser closes
app.config["SESSION_USE_SIGNER"] = True  # Sign session cookies for security
app.config['SESSION_KEY_PREFIX'] = 'GHCL_chatbot:'  # Prefix for Redis keys
app.config["SESSION_REDIS"] = redis.StrictRedis(host="128.91.51.73", port=6379, db=1,decode_responses=False)
Session(app)  # Required for flash messages
...
def get_files_from_google_drive():
    try:
        ...
        service = build('drive', 'v3', credentials=credentials)
        FOLDER_ID = '1zfneSAiCrks-5zVr9agPNufnuU-fmd2p'
        try:
...
def sql_connection():
    ...
    connection_string = 'postgres://postgres:postgres@128.91.51.73:5432/ghclMarketting'
...
def get_ans_from_csv():
    ...
    try:
        ...
        if query_text:
            ...
            #Chain creation
            chain = get_chain(gemini_model, prompt, retriever)
            #result = get_answer(chain,query_text,email,file_name)
            answer_dict = chain.invoke({'question': query_text,'chat_history':''})
            answer = answer_dict['answer']
            print('answer.............,...............,.....................:',answer)
        else:
            raise QuestionNotExistError()

        del email,query_text,file_name,file_with_id_dict,doc_id
        gc.collect()

        return jsonify({'answer': answer_dict['answer']})

    except (QuestionNotExist,Exception) as e:
...
def save_feedback():
    ...
    sql_query = f"insert into public.user_feedback(email_id,file_name,question,feedback,answer,created_at) values ('{email}','{feedback_data['selected_file']}','{feedback_data['question']}','{feedback_data['feedback']}','{feedback_data['answer']}','{time.ctime()}');"
    query = """
    INSERT INTO public.user_feedback(
        email_id, file_name, question, feedback, answer, created_at
    )
    VALUES (%s, %s, %s, %s, %s, %s); """

    params = (
    email,
    feedback_data['selected_file'],
